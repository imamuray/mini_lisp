...
lval* lval_copy(lval* v) {
  ...
  switch (v->type) {
    /* Copy Functions adn Numbers Directly */
    case LVAL_FUN:
      x->fun = v->fun;
      break;
    case LVAL_NUM:
...
void lenv_put(lenv* e, lval* k, lval* v) {
  for (int i = 0; i < e->count; i++) {
    /* If variables is found delete item at that position */
    /* And replace with variable suplied by user */
    if (strcmp(e->syms[i], k->sym) == 0) {
      lval_del(e->vals[i]);
...
lval* builtin_def(lenv* e, lval* a) {
  ...
  /* Ensure all elements of first list are symblos */
